...
import { composeContext, generateText, IAgentRuntime, Memory, ModelClass } from "@elizaos/core";
import { SearchMode } from "agent-twitter-client";
import { shouldAddStartup, startupSumUpNew, startupSumUpOld } from "./prompts";
...
export function getRandomDateRange(): { sinceDate: string; untilDate: string } {
    const start = new Date(2020, 0, 1).getTime(); // 2020-01-01
  ...
  export async function processStartupCandidate(
    ...
    try {
      ...
      if (finalAnswer === "true") {
        ...
        const oldestTweetsRaw = await client.fetchOldest50Tweets(userInfo.id);
        ...
        const latestTweetsResponse = await client.fetchUserPosts(userInfo.id, 50);
